...
**Auth gate (real mode only).** `src/main.jsx` renders `src/AuthGate.jsx`, not the shell directly. In mock mode `AuthGate` passes straight through to `TwoDoShell`. With real Supabase it shows a **magic-link sign-in** (`src/lib/auth.js`), then on first login calls the `bootstrap_space` security-definer RPC (in `schema.sql`) to create the space + two `person` rows linked to `auth.uid()` — needed because RLS (`app_space_id()`) keys off `person.auth_user_id`, a chicken-and-egg the RPC resolves. Don't gate mock mode behind auth.
